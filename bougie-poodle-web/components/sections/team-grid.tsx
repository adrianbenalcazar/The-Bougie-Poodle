    ...
    <FadeInStagger className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
      {TEAM.map((member) => (
        <FadeIn key={member.name} className="rounded-2xl border border-sand/70 bg-white p-7 text-center">
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-heading font-display text-lg text-cream">
            {member.initials}
          </div>
          <h3 className="mt-4 font-display text-lg">{member.name}</h3>
          <p className="mt-1 text-xs font-medium uppercase tracking-wide text-stone">{member.role}</p>
